...
import { RestaurantPickerFunction } from '../Services/DataServices';
...
export default function RestaurantPickerComponent(){
  ...
  const handleUserInput = async ()=>{
    ...
    const pattern = ["mexican", "american", "chinese"];
    if(pattern.includes(userInput.toLowerCase())){
      console.log(pattern)
  
      let response = await RestaurantPickerFunction(userInput.toLowerCase())
    ...
    <Container fluid className = 'p-0'>
      ...
      <div>
        ...
        <Row>
          <Col className='H1-Header'>
          <p>Type either mexican, american or chinese to surface a randomized restaurant to eat at!</p>
          </Col>
        </Row>
      </Container>
      </div>
      <div>
        <Container>
          <Row>
            <Col>
            <Form.Control onChange={(e) => setUserInput(e.target.value)} className="TextBar-Size" type="text" placeholder="Type mexican, american or chinese..."/>
            </Col>
          </Row>
          <Row>
